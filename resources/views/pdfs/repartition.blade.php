...
<head>
    <meta charset="UTF-8">
    <title>Liste d'affichage - {{ $displayLabel ?? ($examLabel ?? ($examen->module->nom_module ?? 'Examen')) }}</title>
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .header { text-align: right; font-size: 10px; margin-bottom: 2px; color: #555; font-weight: bold; }
        .date { font-size: 11px; font-weight: bold; }
        h1 { text-align: center; font-size: 24px; font-weight: 800; margin-bottom: 4px; letter-spacing: 1px; }
        h2 { text-align: center; font-size: 15px; font-weight: 800; margin-bottom: 8px; }
        .info-table { width: 100%; border-collapse: collapse; margin-bottom: 10px; }
        .info-table td { border: 2px solid #000; padding: 6px; font-size: 12px; font-weight: 600; }
        ...
        th { background: #FFD966; text-align: center; font-weight: 800; }
        ...
        .student-name { font-size: 11px; line-height: 1.2; font-family: "DejaVu Sans", Helvetica, Arial, sans-serif; font-weight: bold; flex: 1 1 auto; min-width: 0; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }
...
<body>
    @php
        ...
        $formatCodeGrille = function ($value): string {
            $grille = trim((string) ($value ?? ''));

            if ($grille === '') {
                return '-';
            }

            return \App\Support\CodeGrille::normalize($grille);
        };
                            ...
                            @foreach($columns as $column)
                                @switch($column)
                                    ...
                                    @case('etudiant')
                                        ...
                                        @break
                                    @case('grille')
                                        <td class="text-center">{{ $formatCodeGrille($rep->code_grille ?? null) }}</td>
                                        @break
                                    @case('place')
