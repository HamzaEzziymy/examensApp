...
const buildPayload = (source, overrides = {}) => ({
    ...
    effectif_prevu: overrides.effectif_prevu ?? source.effectif_prevu ?? '',
    bareme_salle: overrides.bareme_salle ?? source.bareme_salle ?? '',
                    ...
                    <div className="w-full max-w-3xl rounded-2xl bg-white p-6 shadow-2xl dark:bg-gray-800">
                        ...
                        <form onSubmit={handleSubmit} className="grid gap-4 lg:grid-cols-2">
                            ...
                            <div className="grid gap-4 lg:col-span-2 sm:grid-cols-2">
                                <div>
                                    <label className="text-sm font-medium text-gray-700 dark:text-gray-200">Effectif attendu</label>
                                    <input
                                        type="number"
                                        min="1"
                                        value={data.effectif_prevu}
                                        onChange={(event) => setData('effectif_prevu', event.target.value)}
                                        className="mt-1 w-full rounded-lg border border-gray-300 bg-transparent px-3 py-2 text-sm focus:border-indigo-500 focus:outline-none focus:ring-indigo-500 dark:border-gray-700"
                                    />
                                    <InputError message={errors.effectif_prevu} className="mt-1" />
                                </div>
                                <div>
                                    <label className="text-sm font-medium text-gray-700 dark:text-gray-200">Bareme par salle</label>
                                    <input
                                        type="number"
                                        min="1"
                                        value={data.bareme_salle}
                                        onChange={(event) => setData('bareme_salle', event.target.value)}
                                        className="mt-1 w-full rounded-lg border border-gray-300 bg-transparent px-3 py-2 text-sm focus:border-indigo-500 focus:outline-none focus:ring-indigo-500 dark:border-gray-700"
                                    />
